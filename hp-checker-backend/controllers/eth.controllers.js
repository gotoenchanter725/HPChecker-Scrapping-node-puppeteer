...
exports.honey_check = async (req, res, next) => {
  ...
  await check_honey(contract_address, (status, data) => {
    if (status == "1") {
      res.send({
        status: status,
        data: data,
...
const check_honey = async (contract_address, result) => {
  ...
  if (parseInt(uniswap_balance_v2) === 0 && parseInt(uniswap_balance_v3) === 0) {
    await DB_honeypotSet(contract_address, true);
...
const get_holder_list = async (contract_address, result) => {
  ...
  try {
    ...
    await page.goto(`https://etherscan.io/token/generic-tokenholders2?m=normal&a=${contract_address}&s=${totalSupply}&p=1`);
    try {
...
const get_contract_api = async (contract_address, result) => {
  const url = `https://api.etherscan.io/api?module=contract&action=getabi&address=${contract_address}&apikey=${ETHSCAN_API_KEY}`;
  await delay(500);
  await axios.get(url)
    .then(response => {
...
const get_total_supply = async (contract_address, result) => {
  const url = `https://api.etherscan.io/api?module=stats&action=tokensupply&contractaddress=${contract_address}&apikey=${ETHSCAN_API_KEY}`;
  await delay(500);
  await axios.get(url)
    .then(response => {
      result(response.data.status, response.data.result);
...
const get_circulating_supply = async (contract_address, result) => {
  const url = `https://api.etherscan.io/api?module=stats&action=tokenCsupply&contractaddress=${contract_address}&apikey=${ETHSCAN_API_KEY}`;
...
const get_soruce_code = async (contract_address, result) => {
  const url = `https://api.etherscan.io/api?module=contract&action=getsourcecode&address=${contract_address}&apikey=${ETHSCAN_API_KEY}`;
...
const get_circulating_supply_width_another_address = async (contract_address, another_address, result) => {
  const url = `https://api.etherscan.io/api?module=account&action=tokentx&contractaddress=${contract_address}&address=${another_address}&offset=10&apikey=${ETHSCAN_API_KEY}`;
...
async function DB_honeypotSet(contract_address, value) {
  ...
  if (!arr) {
    ...
    await History.updateOne({ address: contract_address }, { honeypot: value, net: "eth" })
  }
}
async function DB_rugpullSet(contract_address, value) {
  ...
  if (!arr) {
    ...
    await History.updateOne({ address: contract_address }, { rugpull: value, net: "eth" })
  }
}